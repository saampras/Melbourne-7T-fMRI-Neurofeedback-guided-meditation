...
sca;
clear all
global C_feedback_values TR port_issue window scr_rect centreX centreY escapeKey start_time current_TBV_tr ROI_mean_PSC ROI_vals FirstKey SecondKey PSC_thresh feedback_error;

%% Make changes here

pp_no = 28;
match = 8; %Experimental participant ID to get stored feedback scores from
pp_name = 'Control';
run_no = 9;
run_number = 'Nine';
Day = 2;
...
matched_pp_dir = ['.\Participant_' num2str(match) '\'];
load([matched_pp_dir 'run_' num2str(run_no) '_PSC_values.mat']);
C_feedback_values = ROI_mean_PSC(:,2);
...
rect_num = calculate_feedback(Medtrial_start_tr,Medtrial_dur_tr,folder_path,file_prefix);
...
function curr_feedback = calculate_feedback(medtrial_start,medtrial_dur,folder_path,file_prefix)
...
global ROI_mean_PSC PSC_thresh ROI_vals current_TBV_tr feedback_error C_feedback_values
persistent med_block_num

if isempty(med_block_num)
    med_block_num = 0;
end

med_block_num = med_block_num + 1;
fake_feedback = C_feedback_values(med_block_num); %Loading feedback value from matched participant
...
if first_tr < current_TBV_tr %TBV should have surpassed the first actual TR by now
    ...
    ROI_mean_PSC = [ROI_mean_PSC;med_trial_psc,curr_feedback,first_tr,last_tr,conf_mean,fake_feedback]; %first and last TRs used for calculation
    
    curr_feedback = fake_feedback; %This is what gets fed back
